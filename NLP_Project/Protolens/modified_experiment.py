...
#import torch.nn.functional as F_toxic
...
def calculate_toxicity_penalty(batch_texts, toxicity_model, toxicity_tokenizer, device, toxicity_weight=0.1):
    if not batch_texts or len(batch_texts) == 0:
        return torch.tensor(0.0, device=device)

    toxicity_model.eval() # Set toxicity model to evaluation mode for inference

    # Tokenize the input texts for the toxicity model
    toxic_inputs = toxicity_tokenizer(
        batch_texts,
        return_tensors='pt',
        padding=True,
        truncation=True,
        max_length=128 # Use max_length consistent with toxicity model training
    ).to(device)

    with torch.no_grad():
        toxic_outputs = toxicity_model(**toxic_inputs)
        # Assuming multi-label classification for toxicity, get probabilities
        toxic_probs = torch.sigmoid(toxic_outputs.logits) # Shape: (batch_size, num_toxicity_labels)

        # A simple penalty: average the probabilities of all toxic labels.
        # This penalizes any form of toxicity predicted by your model.
        mean_batch_toxicity = toxic_probs.mean(dim=1) # Average prob per text in batch
        penalty = mean_batch_toxicity.mean() # Average across the batch

    toxicity_model.train() # Set toxicity model back to train mode if it's going to be used in ProtoLens's training

    return toxicity_weight * penalty

def train_step(model, val_dataloader, data_loader, optimizer, scheduler, device, new_proto, tau=1, train_texts=None, toxicity_model=None, toxicity_tokenizer=None, toxicity_weight=0.1):
    model.train()
    Loss = 0
    ...
    for (batch_num,batch) in enumerate(data_loader):
        ...
        #     LOG=False
        if batch_num % 100 == 0:
            print(f"Batch {batch_num} / {len(data_loader)}")
        LOG=False
        optimizer.zero_grad()
        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        ...
        # --- Toxicity Penalty Integration ---
        if toxicity_model is not None and toxicity_tokenizer is not None:
            # Assuming 'original_text' is a list of strings available in the batch
            batch_texts_for_toxicity = [t for t in original_text] # Ensure it's a list of strings
            toxicity_penalty = calculate_toxicity_penalty(batch_texts_for_toxicity, toxicity_model, toxicity_tokenizer, device, toxicity_weight)
            loss = loss + toxicity_penalty # Add penalty to the main loss
            logging.info(f"Toxicity Penalty: {toxicity_penalty.item():.4f}")
        # -------------------------------------
     #+ 0.1 * augmented_loss#+ span_loss#+ att_loss.long() + augmented_loss #+ augmented_loss#+ 0.0001 * diversity_loss#+  0.0001 *  locality_loss# + 0.00001 * cLoss #+ 0.001 * eLoss
...
def train_model(gpu, args, toxicity_model, toxicity_tokenizer, toxicity_weight=0.1):
    ...
    for epoch in range(num_epochs):
        ...
        Loss = train_step(model,val_dataloader, train_dataloader, optimizer, scheduler, device, new_proto, tau=tau, train_texts=train_texts, toxicity_model=toxicity_model, toxicity_tokenizer=toxicity_tokenizer, toxicity_weight=toxicity_weight)
        ...
        if accuracy > best_result: 
            ...
            torch.save({'model_state_dict': model.state_dict(), 'pnfrl_args': pnfrl_args}, 'protolens_fair.pth')
...
def train_main(args):
    ...
    toxicity_tokenizer = BertTokenizer.from_pretrained('../Saved_model_fair')
    toxicity_model = BertForSequenceClassification.from_pretrained('../Saved_model_fair')

    # Move the model to the appropriate device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    toxicity_model.to(device)
    train_model(0, args, toxicity_model, toxicity_tokenizer)
